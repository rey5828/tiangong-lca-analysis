...
INDEX_NAME = "processsdep_validation_index1"
BATCH_SIZE = 5000
SCROLL_TIME = "2m"
...
def get_process_documents():
    query = {
        "_source": ["process_name", "process_id", "flow_id", "ratio", "flow_type"],
        "query": {"match_all": {}}
    }
...
def process_duplicates():
    ...
    for doc in get_process_documents():
        ...
        # Only store ELEMENTARY_FLOW and WASTE_FLOW
        if (process_id and first_part and flow_id is not None and 
            flow_type in ['ELEMENTARY_FLOW', 'WASTE_FLOW']):
