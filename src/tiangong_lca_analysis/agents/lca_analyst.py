...
1. Shared Driver (Positive): Both flows must be directly generated by the same formation mechanism, i.e. the same explicit physico-chemical generation pathway (e.g. a specific chemical reaction or volatilisation mechanism) that is necessary for the formation of both flows (e.g., Combustion generates CO2 and releases Mercury from coal). Co-location in the same equipment or process stage alone is not sufficient. If the formation of one flow can be conceptually varied without directly altering the formation mechanism of the other at fixed functional unit and intensity, the relationship must not be classified as Shared Driver.
2. Trade-off (Negative): Abatement of the pollutant consumes energy/chemicals or materials, leading to additional GHG emissions (e.g., Scrubber removes SO2 but uses electricity -> Indirect CO2).
...
4. Decoupled (Neutral): The pollutant and GHG originate from mechanistically independent pathways, with no shared reaction/formation mechanism, control parameter, or energy-based coupling (e.g., Noise vs Combustion CO2).
