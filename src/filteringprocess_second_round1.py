...
class ProcessDeduplicator:
    ...
    def merge_same_flows(self, process_flows: List[dict]) -> List[dict]:
        ...
        for flow_id, group in flow_groups.items():
            if len(group) > 1:
                # 如果有多个相同flow_id的文档，合并它们
                base_flow = group[0].copy()
                total_amount = sum(flow['flow_amount'] for flow in group)
                base_flow['flow_amount'] = total_amount
    ...
    def get_process_fingerprint(self, flows: List[dict]) -> str:
        """生成process的指纹，只考虑ELEMENTARY_FLOW和WASTE_FLOW"""
        filtered_flows = [
            flow for flow in flows 
            if flow.get('flow_type') in ['ELEMENTARY_FLOW', 'WASTE_FLOW']
        ]
        sorted_flows = sorted(filtered_flows, key=lambda x: (x['flow_id'], x['flow_name']))
        fingerprint_parts = []
    ...
    def detect_scaling_processes(self, processes: Dict[str, List[dict]]) -> Set[str]:
        ...
        for process_id, flows in processes.items():
            ...
            process_name = filtered_flows[0]['process_name']
            first_part = process_name.split('|')[0].strip()
            name_groups[first_part].append(process_id)
            process_flows[process_id] = filtered_flows
        ...
        for group in name_groups.values():
            ...
            for i in range(len(group)):
                for j in range(i + 1, len(group)):
                    process_id1 = group[i]  # Get process_id directly
                    process_id2 = group[j]
                    flows1 = process_flows[process_id1]  # Get flows from process_flows
                    flows2 = process_flows[process_id2]
                    
                    # 创建用于结构比较的集合
                    structure1 = {(f['flow_id'], f['flow_name']) for f in flows1}
                    structure2 = {(f['flow_id'], f['flow_name']) for f in flows2}
                    
                    # 首先检查流程结构是否完全相同
                    if structure1 == structure2:
                        # 创建flow_id到flow的映射
                        flows1_dict = {f['flow_id']: f for f in flows1}
                        flows2_dict = {f['flow_id']: f for f in flows2}

                        # 计算每个对应flow的比例
                        ratios = []
                        for flow_id in flows1_dict:  # 因为结构相同，可以只遍历一个字典的键
                            amount1 = flows1_dict[flow_id]['flow_amount']
                            amount2 = flows2_dict[flow_id]['flow_amount']
                            if amount1 != 0 and amount2 != 0:
                                ratios.append(amount2 / amount1)
                        #检查比例是否恒定
                        if ratios and np.std(ratios) < 1e-6:
...
def main():
    ...
    index_name = "processwithghg_dedup1"
    
    # 创建去重器实例，设置适当的批处理大小
    deduplicator = ProcessDeduplicator(es, index_name, batch_size=5000)
    
    # 执行去重
    deduplicator.deduplicate(new_index_name="processwithghg_dedup7")
